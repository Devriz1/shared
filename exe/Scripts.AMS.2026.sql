...
if @No is not null
	...
	join aAccountSettings s on s.CompanyId=@CompanyId
	join aAccount a on s.Commission=a.Id 
	left join aTaxGroup t on a.TaxGroupId=t.Id
	left join aCreditCard c on cd.CardID=c.ID
	where cd.CompanyID=@CompanyID and cd.PeriodID=@PeriodID and  cd.No=@No

go

if object_id('spGetAllAccountTransaction') is not null drop proc spGetAllAccountTransaction

go

create proc spGetAllAccountTransaction
@CompanyID int,
@PeriodID int,
@FromDate int,
@ToDate int,
...
Create table #Transaction(CompanyID int,PeriodID int,Date smalldatetime,VoucherTypeID int,VType varchar(100)
,No int,SNo int,RefNo varchar(100),Debtor nvarchar(100),DebtorID int,
Creditor nvarchar(100),CreditorID int,AccountID int,IAccount nvarchar(100),CNo varchar(25),CDate int,Description nvarchar(200),CreditCardID int,CreditCard nvarchar(50),Amount float)

	insert #Transaction(CompanyID,PeriodID,Date,VoucherTypeID,No,SNo,RefNo,DebtorID,CreditorID,AccountID,
	CNo,CDate,CreditCardID,Description,Amount)
	select t.CompanyID,t.PeriodID,cast(cast(t.Date as nvarchar)as smalldatetime)Date,t.VoucherTypeID,
	t.No,t.SNo,t.RefNo,t.DebtorID,
	t.CreditorID,t.AccountID,CNo,CDate,t.CreditCardID,
	t.Description,t.Amount from aTransaction t
	where (@CompanyID=0 or t.CompanyID=@CompanyID or t.CompanyID is null) 	 
	and t.Date between @FromDate and @ToDate
	and @DebtorID= case when @DebtorID=0 then 0 else t.DebtorID end
	and @CreditorID= case when @CreditorID=0 then 0 else t.CreditorID end
	and @CreditorID= case when @CreditorID=0 then 0 else t.CreditorID end
	and @VtypeID= case when @VtypeID=0 then 0 else t.VoucherTypeID end
	and isnull(Description,'')  like '%'+@Description+'%'
	and @CreditCardID= case when @CreditCardID=0 then 0 else t.CreditCardID end
	and @IAccount= case when @IAccount=0 then 0 else t.AccountID end
	and (@CostCentreId=0 or t.CostCentreId=@CostCentreId)
	order by date
	
    update t set t.VType=V.name from #Transaction t join aVoucherType v on t.VoucherTypeID=v.ID 
    update t set t.Debtor=a.name from #Transaction t join aAccount a on t.DebtorID=a.ID 
	update t set t.Creditor=a.name from #Transaction t join aAccount a on t.CreditorID=a.ID 
	--IAccount
	update l set l.IAccount=a.Name from #Transaction l join aAccount a on l.AccountID=a.ID
	update l set l.CreditCard=a.Name from #Transaction l join aCreditCard a on l.CreditCardID=a.ID

--Cheque Details
update #Transaction set Description='Cheque No.'+CNo+' - '+'Date.'+convert(varchar,cast(cast(CDate as varchar) as smalldatetime),3)+' - '+isnull(Description,'') where len(rtrim(CNo))>0

declare @Line varchar(25)
set @Line=REPLICATE('-',25)

   select CompanyID,PeriodID,Date,VoucherTypeID,VType,No,SNo,RefNo,Debtor,Creditor,IAccount,CreditCard,Description,convert(varchar,cast(Amount as money),4)Amount from #Transaction
    
	union all
		select null,null,null,null,null,null,null,null,null,null,null,null,null,@Line
		union all
		select null,null,null,null,null,null,null,null,null,null,null,null,null,convert(varchar,cast(sum(Amount) as money),4) from #Transaction
		union all
		select null,null,null,null,null,null,null,null,null,null,null,null,null,@Line
